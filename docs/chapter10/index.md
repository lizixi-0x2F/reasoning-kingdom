...
**算法1:MCTS主循环**
    ...
    3. # 选择访问次数最多的动作
       a* = argmax(child.visits for child in root.children)
...
**算法2:UCB选择**
    ...
    best_child_UCB(node, c=√2):
    输入: 当前节点, 探索常数 c
    输出: UCB分数最高的子节点

    1. 对每个子节点 child:
       利用项 = child.value / child.visits
       探索项 = c × √(ln(node.visits) / child.visits)
       UCB(child) = 利用项 + 探索项

    2. 返回 argmax(UCB(child))

**算法3:ADS熵正则化**

    ADS-Entropy-Regularization(LLM, 输入x, 最大步数T):
    输入: 语言模型LLM, 输入问题x, 最大推理步数T
    输出: 推理路径和最终答案
    ...
    2. for t = 1 to T:
       # 计算当前输出分布的熵
       p_t = LLM(s_t)
       H_t = -Σ p_t(a) × log p_t(a)
       H_max = log |A|  # 最大熵(均匀分布)

       # 动态对数障碍系数
       α_t = -log(1 - H_t / H_max)

       # 熵正则化损失
       loss = CrossEntropy(p_t, target) + α_t × H_t
