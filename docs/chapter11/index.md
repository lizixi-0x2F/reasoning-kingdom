...
**算法1:Collins Count-Sketch压缩**
    ...
    Collins-Adam(参数θ, 梯度g, 压缩比c):
    输入: 参数θ ∈ R^d, 梯度g ∈ R^d, 压缩比c
    输出: 更新后的参数θ
    ...
    1. 初始化(首次调用):
       k = d / c  # 压缩后的维度
       sketch_m = 零向量(k)  # 一阶矩sketch
       sketch_v = 零向量(k)  # 二阶矩sketch
       h1, h2 = 随机哈希函数(2-Universal)

    2. 压缩梯度到sketch:
       for i = 1 to d:
         j = h1(i) mod k
         sign = h2(i) ∈ {-1, +1}
         sketch_m[j] += sign × β1 × m[i] + (1-β1) × g[i]
         sketch_v[j] += sign × β2 × v[i] + (1-β2) × g[i]²

    3. 恢复近似矩估计:
       for i = 1 to d:
         j = h1(i) mod k
         sign = h2(i)
         m_approx[i] = sign × sketch_m[j]
         v_approx[i] = sign × sketch_v[j]

    4. Adam更新:
       θ[i] ← θ[i] - η × m_approx[i] / (√v_approx[i] + ε)

    5. Chernoff保证:
       P(||m - m_approx|| > ε||m||) < 2exp(-ε²k·T_eff / 2dB²)
       当c ≤ c_opt ≈ 34时,误差可控
...
**算法2:能量/逻辑度量**
    ...
    Energy-per-Logic(推理任务, 模型):
    输入: 推理任务(如解方程), 模型(符号/神经网络)
    输出: 每逻辑步骤的能量成本
    ...
    2. 测量每步的FLOPs:
       for step in steps:
         flops[step] = 计数(乘法 + 加法 + 其他运算)

    3. 估算能量:
       energy_per_flop = 10^-14 焦耳  # 现代GPU
       total_energy = Σ flops[step] × energy_per_flop

    4. 计算效率:
       efficiency = total_energy / len(steps)  # 焦耳/逻辑步骤

    5. 对比基线:
       human_brain = 10^-16 焦耳/逻辑步骤  # 理论估计
       ratio = efficiency / human_brain

    6. 返回 (efficiency, ratio)
...
- **能量/逻辑度量能否标准化?** 如何公平地比较不同架构、不同任务的推理效能?
